...
import { Save, MapPin, Phone, Mail, Link as LinkIcon, Building2 } from 'lucide-react';

export default function MainPortalFooter() {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem('cms_main_footer');
    if (savedData) {
  ...
  const handleSave = () => {
    localStorage.setItem('cms_main_footer', JSON.stringify(formData));
    window.dispatchEvent(new Event('local-storage-update'));
    setIsSaved(true);
    setTimeout(() => setIsSaved(false), 2000);
  };

  const inputCls = "w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all";
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-200/60 overflow-hidden">
      <div className="border-b border-slate-200 bg-slate-50 p-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage the contact information and map displayed at the bottom of the main portal.</p>
        </div>
        <button 
          onClick={handleSave}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input 
                ...
                className={inputCls} 
                placeholder="e.g. Akshar Academy"
              />
            </div>
            ...
            <div>
              ...
              <input 
                ...
                onChange={handleChange} 
                className={inputCls} 
                placeholder="e.g. Akshara marg, Butwal"
              />
            </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              ></textarea>
              <p className="text-xs text-slate-400 mt-2">Go to Google Maps &gt; Share &gt; Embed a map &gt; Copy the URL inside the src="..." attribute.</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Map Preview</h4>
              <div className="w-full h-32 rounded-lg overflow-hidden border border-slate-200 bg-slate-200">
                {formData.mapUrl ? (
                  <iframe
                    title="Map Preview"
                    src={formData.mapUrl}
