...
import { Calendar, Upload, Save, Edit3, Check } from 'lucide-react';

export default function SchoolEventsHero() {
    const [title, setTitle] = useState("Events & Happenings");
    const [subtitle, setSubtitle] = useState("Stay updated with vibrant school programs, competitions, and milestone celebrations.");
    const [bgImage, setBgImage] = useState("https://images.unsplash.com/photo-1511578314322-379afb476865?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80");
    const [isSaved, setIsSaved] = useState(false);

    useEffect(() => {
        const saved = localStorage.getItem('cms_school_events_hero');
        if (saved) {
    ...
    const handleSave = () => {
        localStorage.setItem('cms_school_events_hero', JSON.stringify({ title, subtitle, bgImage }));
            ...
            <div className="border-b border-slate-100 pb-5 mb-8 flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                        ...
                        School - Events Hero Section
                    </h2>
                    <p className="text-sm text-slate-500 mt-1">Manage banner for the Events page.</p>
                </div>
                <button 
                    onClick={handleSave}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                <div>
                    ...
                    <div className="relative group rounded-2xl overflow-hidden border-2 border-dashed border-slate-300 aspect-video bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer flex flex-col items-center justify-center text-center">
                        {bgImage ? (
                            <img src={bgImage} alt="Events Hero" className="w-full h-full object-cover" />
                        ) : (
                            <Upload className="w-8 h-8 text-slate-400" />
                        )}
                        <label htmlFor="school-events-hero" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center cursor-pointer text-white">
                            <Upload className="w-8 h-8 mb-2" />
                            <span className="text-sm font-bold">Change Background Image</span>
                        </label>
                        <input id="school-events-hero" type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                    </div>
                </div>
