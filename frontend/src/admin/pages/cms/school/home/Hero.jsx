...
export default function SchoolHomeHero() {
    const [title, setTitle] = useState("Trusted by more than 50000+ students");
    const [subtitle, setSubtitle] = useState('"Be a part of this amazing institution"');
    const [bgImage, setBgImage] = useState("https://images.unsplash.com/photo-1509062522246-3755977927d7?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80");
    const [isSaved, setIsSaved] = useState(false);

    useEffect(() => {
        const saved = localStorage.getItem('cms_school_home_hero');
        if (saved) {
    ...
    const handleSave = () => {
        localStorage.setItem('cms_school_home_hero', JSON.stringify({ title, subtitle, bgImage }));
            ...
            <div className="border-b border-slate-100 pb-5 mb-8 flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                        ...
                        School - Home Hero Section
                    </h2>
                    <p className="text-sm text-slate-500 mt-1">Manage the school homepage background banner and welcome headline.</p>
                </div>
                <button 
                    onClick={handleSave}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                <div>
                    ...
                    <div className="relative group rounded-2xl overflow-hidden border-2 border-dashed border-slate-300 aspect-video bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer flex flex-col items-center justify-center text-center">
                        ...
                        <label htmlFor="school-hero-upload" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center cursor-pointer text-white">
                            <Upload className="w-8 h-8 mb-2" />
                            <span className="text-sm font-bold">Change Background Image</span>
                        </label>
                        <input id="school-hero-upload" type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                    </div>
                    <p className="text-xs text-slate-400 mt-2">Recommended resolution: 1920x800. Supports JPG, PNG, WebP.</p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Sub Heading</label>
                        <textarea 
                            rows={4} 
                            value={subtitle} 
                    ...
                    <div className="p-4 bg-emerald-50/60 rounded-2xl border border-emerald-100">
                        <p className="text-xs text-emerald-800 font-medium">
                            Live Preview Text: <strong>{title}</strong> — {subtitle}
                        </p>
                    </div>
                </div>
            </div>
